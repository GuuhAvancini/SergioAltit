...
<p>Pela presente, o acima qualificado, devorante denominado comitente, autoriza o Leiloeiro Sérgio Altit, credenciado na  JUCESP  sobra a matricula 440, situado na Rua Amélia Correia Fontes Guimarães, 49, Vila Progredior, CEP 05617-010, São Paulo, SP, a vender os bens de sua propriedade, abaixo relacionadas, nos termos e condições a seguir:</p>
<ol>
  <li>A(s) obra(s) de arte, objetos da presente autorização se encontram livres e desembaraçadas de quaisquer ônus, responsabilizando-se o Comitente/Proprietário por si e seus sucessores, perante <? if($_POST['tipoConsig'] == "ag"){?>a Galeria<? }else{?>ao Leiloeiro <? }?> e Terceiros, pela autenticidade e procedência das mesmas;</li>
  <li>As partes estabelecem para venda das obras o preço mínimo fixado após a descrição das mesmas;</li>
  <li>O Comitente/ Proprietário autoriza desde de já o Leiloeiro a deduzir a título de reembolso de despesas o percentual de ( &nbsp;&nbsp;&nbsp; <strong>%)</strong> sobre o valor das obras vendidas.</li>
  <li>A liquidação da(s) obra(s) será feita após pagamento dos compradores, no prazo de 15 dias úteis após a venda. Sendo total responsabilidade <? if($_POST['tipoConsig'] == "ag"){?>da Galeria<? }else{?>do Leiloeiro <? }?>;</li>
  <li>Fica <? if($_POST['tipoConsig'] == "ag"){?>a Galeria autorizada<? }else{?>o Leiloeiro autorizado<? }?> a divulgar o nome do proprietário das obras quando achar necessário, inclusive reproduzir as mesmas em catálogo, jornais ou qualquer outra mídia;</li>
  <li>Esta autorização é dada em caráter irrevogável e irretratável, pelo prazo de 180 dias</li>
  <li>Fica desde logo eleito o Foro de São Paulo, para dirimir eventuais divergências da presente;</li>
  <li>Fica autorizado ao Leiloeiro a circulação da(s) obra(s) <? if($_POST['tipoConsig'] == "ag"){?>entre os dois escritórios sendo um em São Paulo e o outro no Guarujá<? }else{?>
    dentro de São Paulo<? }?>.</li>
</ol>
