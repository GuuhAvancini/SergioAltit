...
<div id="conteudo">
    ...
    <div id="searchPaginas">
        <form action="?op=buscaCadastro&idPeca=<?=$_GET['idPeca']?>" method="POST">
		...
		?>
    <div  style="padding-top:15px; float:left; padding-left:20px;"><strong><a href="?inicial=<? echo $linha1['iniciais'];?>&op=buscaCadastro&tipoBusca=inicio&idPeca=<?=$_GET['idPeca']?>" style="font-size:15px; text-decoration:none; color:#999"><? echo strtoupper($linha1['iniciais']);?></a></strong></div>
    <? }?>
        </form>
    </div>
    </div>
    ...
    <table width="90%" border="0" cellpadding="0" cellspacing="0" class="tabelaBusca"  style="margin-top:130px !important;">
      ...
      <tr bgcolor="#fffdea">
        ...
        <td style="border-right:none !important; color:#787878 !important;" ><input type="button" value="Alterar Comitente" name="new_button" id="new_button" onclick="location.href='altera-comitente-vendedor-ok.php?idPeca=<?=$_GET['idPeca']?>&idCadastro=<?=$linha['idCadastro']?>'"></td>
      </tr>
      <?
		  }
	  }else{
    ...
    <? }?><input type="button" value="Voltar para Acervo" name="new_button" id="new_button" onclick="location.href='view-acervo.php?idPeca=<?=$_GET['idPeca']?>'" style="margin-left:220px; margin-top:-10px; position: absolute; z-index:100;">
</div>
<? if($_GET['op'] == "cadok"){?>
