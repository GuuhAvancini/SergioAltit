...
function validaForm(){
	...
	if(obj.cpf.value == ""){
		alert("Campo de CPF/CNPJ vazio, favor preencher.")
		obj.cpf.focus()
		return
	}
...
<div id="conteudo">
    ...
    <div id="cadastro" align="right">
        <form action="edita-comitente-ok.php?idCadastro=<?=$_GET['idCadastro']?>" method="POST" id="form1" name="form1">
            <p>Nome Completo:
...
<input type="text" name="cidade" id="cidade" style="width:200px;" class="inputsTxt"  value="<?=$linhaEc['cidade']?>"/> Estado: <input name="estado" type="text" id="estado" style="width:50px;" maxlength="2" class="inputsTxt" value="<?=$linhaEc['estado']?>"/><br />
            ...
            <p>
              Banco: 
              <input type="text" name="banco" id="input_text" style="width:155px;" value="<?=$linhaEc['banco']?>"/> 
              Agência: 
              <input type="text" name="agencia" id="input_text" style="width:120px;" value="<?=$linhaEc['agencia']?>"/> Conta: 
              <input type="text" name="conta" id="input_text" style="width:120px;" value="<?=$linhaEc['conta']?>"/><br />
              Tipo de conta: <select name="tipoConta" id="input_text" style="width:135px;"> 
                <option value="Conta Corrente" <? if($linhaEc['tipoConta'] == "Conta Corrente"){?> selected="selected"<? }?>>Conta Corrente</option>
                <option value="Conta Poupança" <? if($linhaEc['tipoConta'] == "Conta Poupança"){?> selected="selected"<? }?>>Conta Poupança</option>
              </select> Favorecido: 
              <input type="text" name="favorecido" id="input_text" style="width:290px;" value="<?=$linhaEc['favorecido']?>"/><br />
              CPF/CNPJ: 
              <input type="text" name="cpfFavorecido" id="input_text" style="width:260px; margin-right:250px;" value="<?=$linhaEc['cpfFavorecido']?>"/>
            </p>
            <p>
              <input type="button" value="Alterar" name="envia_button" id="envia_button" onClick="javascript:validaForm()">
            </p>
        </form>
    </div>
    </div>
