...
<div id="conteudo">
    ...
    <div style="float:right"><input type="button" value="Gerar Excel" name="new_button" id="new_button" onclick="window.open('excel-compradores.php')" style="margin-right:70px; margin-top:-10px; position:relative; z-index:600;"></div>
    ...
    <div id="searchPaginas">
        <form action="?op=buscaCadastro" method="POST">
            ...
            <input type="button" value="+ Novo Comprador" name="new_button" id="new_button" onclick="location.href='novo-comprador.php'">
            <?
		$query1 = "Select distinct mid(nome,1,1) as iniciais from compradores where codigoCliente = '".$_SESSION['codigoCliente']."' order by mid(nome,1,1)";
    ...
    <table width="90%" border="0" cellpadding="0" cellspacing="0" class="tabelaBusca"  style="margin-top:130px !important;">
	  ...
	  if($_GET['tipoBusca'] == "inicio"){
			$queryCs = "Select * from compradores where nome like '".$_GET['inicial']."%' and codigoCliente = '".$_SESSION['codigoCliente']."' order by nome asc";
		}else{
	  		$queryCs = "Select * from compradores where idComprador like '%".$_POST['search_text']."%' and codigoCliente = '".$_SESSION['codigoCliente']."' or nome like '%".$_POST['search_text']."%' and codigoCliente = '".$_SESSION['codigoCliente']."' or cpf like '%".$_POST['search_text']."%' and codigoCliente = '".$_SESSION['codigoCliente']."' or email like '%".$_POST['search_text']."%' and codigoCliente = '".$_SESSION['codigoCliente']."' order by nome asc";
		}
      $resultadoCs = mysql_query($queryCs);
	  if (mysql_num_rows($resultadoCs)!=0) {
		  while ($linha = mysql_fetch_array($resultadoCs)) {
	  ?>
      <tr bgcolor="#fffdea" onClick="location.href='view-comprador.php?idCadastro=<?=$linha['idCadastro']?>'" style="cursor:pointer;">
        ...
        <td style="border-right:none !important; color:#787878 !important;" ><a href="view-comprador.php?idCadastro=<?=$linha['idCadastro']?>"><img src="images/1464975410_old-edit-find.png" width="25" height="26" border="0" /></a></td>
      </tr>
      <?
		  }
	  }else{
	  ?>
      <tr bgcolor="#fffdea">
        <td colspan="7" style="border-bottom:none !important; border-right:none !important; color:#787878 !important;"><em>Nenhum comprador encontrado...</em></td>
        </tr>
    </table>
    <? }?>
    </div>
...
Cadastro realizado com sucesso! <br /><br />ID Comprador: <?=$_GET['id']?><br />
