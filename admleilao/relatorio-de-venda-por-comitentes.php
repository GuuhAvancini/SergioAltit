...
$linhalog = "- ".date('d/m/Y - H:m:s')." - Usuário: ".$_SESSION['usuarioNome']." - Visualizou o relatório de venda por comitentes.\n";
...
<body>
  ...
  <table width="100%" border="0">
    <tr>
      ...
      <td width="88%"><div id="tituloRelatorio" align="center">RELATÓRIO VENDAS POR COMITENTE - <?=$_SESSION['clienteNome']?><br />
Leilão: <?=$linha['idLeilaoN']?> - <?=$linha['descricao']?> - <?=$linha['dataLeilao']?> - <?=$linha['horario']?></div></td>
    </tr>
  </table>
  <br />
<?
...
<div  >Comitente: <?=$linhaEc['nome']?><br />Telefone: <?=$linhaEc['telefone']?> - Celular: <?=$linhaEc['celular']?> - E-mail: <?=$linhaEc['email']?><br /></div>
	  ...
	  if (mysql_num_rows($resultadoLotesB)!=0) {
		  while ($linhaLotesB = mysql_fetch_array($resultadoLotesB)) {
			...
			$queryComp = "Select * from compradores where idCadastro = '".$linhaLotesB['idComprador']."'";
	...
	if ($linhaLot['foto'] == ""){
		...
		echo $linhaLot['foto'];}?>" width="52" height="56" /></td>
          ...
          <td width="42%" valign="top">Arrematado por:<br />
            <?=$linhaLotesB['cartela']?> - <?=$linhaComp['nome']?><br />
